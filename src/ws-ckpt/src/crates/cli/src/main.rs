...
/// Upper bound for the best-effort advisory IPC. Cap is dominated by the
/// daemon's synchronous `btrfs filesystem usage` subprocess (fork+exec plus
/// btrfs metadata traversal — typically 50-200ms on healthy filesystems,
/// occasionally several hundred ms on busy ones). 10ms missed the window
/// almost every run, so the post-command warning was effectively dead.
/// 500ms keeps a stuck daemon from delaying command exit by more than
/// half a second.
