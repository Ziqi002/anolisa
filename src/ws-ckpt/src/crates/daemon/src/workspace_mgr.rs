    ...
    if abs_path.starts_with(&state.mount_path) {
        ...
        if let Ok(rest) = abs_path.strip_prefix(&state.mount_path) {
            ...
            if let Some(ws_id) = single {
                ...
                // Orphan subvol (e.g. daemon restart lost state). Adopt
                // only if a snapshot bucket exists under snapshots_root —
                // that bucket is created during init, so its presence is
                // the strongest available "this was a real workspace"
                // signal and excludes random user-created directories.
                // Register with the canonical subvol path; subsequent
                // `resolve_workspace` canonicalises to the same path and
                // matches.
                if state.backend.snapshots_root().join(&ws_id).exists() {
                    warn!(
