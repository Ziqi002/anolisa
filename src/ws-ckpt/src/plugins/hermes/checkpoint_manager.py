"""CLI wrapper and snapshot management for the ws-ckpt Hermes plugin.

Wraps all `ws-ckpt` CLI invocations using subprocess.run.
Each method constructs the appropriate CLI arguments, executes the command,
and returns structured results.
"""

from __future__ import annotations

import json
import shutil
import subprocess
from dataclasses import dataclass
from datetime import datetime, timezone
from typing import Any, Dict, List, Optional

from .config import HermesPluginConfig, MSG_TRUNCATE_LEN
from .snapshot_store import SnapshotInfo, SnapshotStore

# Default command execution timeout in seconds.
DEFAULT_TIMEOUT_S = 30

# The ws-ckpt CLI binary name.
WS_CKPT_BIN = "ws-ckpt"


@dataclass
class CommandOutput:
    """Structured output from a CLI invocation."""

    exit_code: int
    stdout: str
    stderr: str


@dataclass
class CheckpointResult:
    """Result of a checkpoint creation attempt."""

    success: bool
    message: str
    snapshot: str = ""
    skipped: bool = False
    reason: Optional[str] = None


def map_error_to_message(stderr: str, context: Optional[Dict[str, Any]] = None) -> str:
    """Map CLI stderr to a user-friendly error message.

    Follows the OpenClaw mapErrorToLLMMessage pattern.
    """
    ctx_str = ""
    if context:
        ctx_str = f" (context: {json.dumps(context)})"

    lowered = stderr.lower()

    if "not initialized" in lowered:
        return f"Workspace not initialized for ws-ckpt.{ctx_str}"
    # CLI environment issues take priority over generic "not found" (snapshot)
    if "binary not found" in lowered or "not found on path" in lowered:
        return f"ws-ckpt CLI not found on PATH.{ctx_str}"
    if "not found" in lowered or "no such" in lowered:
        return f"Snapshot not found.{ctx_str}"
    if "daemon" in lowered or "connection" in lowered:
        return f"ws-ckpt daemon is not responding. Is it running?{ctx_str}"
    if "permission" in lowered:
        return f"Permission denied.{ctx_str}"
    if "timeout" in lowered:
        return f"Command timed out.{ctx_str}"

    return f"ws-ckpt error: {stderr.strip()}{ctx_str}"


class CheckpointManager:
    """Manages ws-ckpt CLI operations and snapshot state.

    Provides synchronous methods for creating checkpoints, listing snapshots,
    querying status, and managing the in-memory snapshot store.
    """

    def __init__(self, config: HermesPluginConfig) -> None:
        self._config = config
        self._store = SnapshotStore()
        self._turn_count: int = 0
        self._cli_available: Optional[bool] = None

    @property
    def turn_count(self) -> int:
        """Current turn counter value."""
        return self._turn_count

    def advance_turn(self) -> int:
        """Increment and return the turn counter."""
        self._turn_count += 1
        return self._turn_count

    def get_store(self) -> SnapshotStore:
        """Return the underlying snapshot store."""
        return self._store

    def get_timeline(self) -> List[SnapshotInfo]:
        """Return snapshots in chronological order (delegates to store)."""
        return self._store.get_timeline()

    def is_cli_available(self) -> bool:
        """Check if the ws-ckpt CLI binary is available on PATH."""
        if self._cli_available is None:
            self._cli_available = shutil.which(WS_CKPT_BIN) is not None
        return self._cli_available

    # ------------------------------------------------------------------
    # CLI execution
    # ------------------------------------------------------------------

    def _run(self, args: List[str]) -> CommandOutput:
        """Execute a ws-ckpt CLI command and return structured output.

        Args:
            args: CLI arguments (excluding the binary name).

        Returns:
            CommandOutput with exit code, stdout, and stderr.
        """
        try:
            result = subprocess.run(
                [WS_CKPT_BIN, *args],
                capture_output=True,
                text=True,
                timeout=DEFAULT_TIMEOUT_S,
            )
            return CommandOutput(
                exit_code=result.returncode,
                stdout=result.stdout,
                stderr=result.stderr,
            )
        except subprocess.TimeoutExpired:
            return CommandOutput(
                exit_code=1,
                stdout="",
                stderr=f"Command timed out after {DEFAULT_TIMEOUT_S} seconds",
            )
        except FileNotFoundError:
            self._cli_available = False
            return CommandOutput(
                exit_code=127,
                stdout="",
                stderr=f"{WS_CKPT_BIN} binary not found on PATH",
            )
        except Exception as e:
            return CommandOutput(
                exit_code=1,
                stdout="",
                stderr=str(e),
            )

    # ------------------------------------------------------------------
    # High-level operations
    # ------------------------------------------------------------------

    def init_workspace(self) -> CommandOutput:
        """Initialize a workspace for ws-ckpt management.

        Equivalent to: ws-ckpt init --workspace <ws>
        """
        return self._run(["init", "--workspace", self._config.workspace_path])

    def create_checkpoint(
        self,
        snapshot_id: str,
        message: str = "",
        metadata: Optional[Dict[str, Any]] = None,
    ) -> CheckpointResult:
        """Create a checkpoint (snapshot) of the workspace.

        Equivalent to:
            ws-ckpt checkpoint --workspace <ws> --id <id> [--message <msg>] [--metadata <json>]

        Args:
            snapshot_id: Caller-provided snapshot identifier.
            message: Optional message describing the checkpoint.
            metadata: Optional metadata dict (serialized as JSON).

        Returns:
            CheckpointResult describing the outcome.
        """
        if not self.is_cli_available():
            return CheckpointResult(
                success=False,
                message="ws-ckpt CLI not found on PATH",
            )

        args = [
            "checkpoint",
            "--workspace", self._config.workspace_path,
            "--id", snapshot_id,
        ]

        # Truncate message to MSG_TRUNCATE_LEN
        if message:
            truncated = message[:MSG_TRUNCATE_LEN]
            args.extend(["--message", truncated])

        if metadata:
            args.extend(["--metadata", json.dumps(metadata)])

        output = self._run(args)

        if output.exit_code != 0:
            return CheckpointResult(
                success=False,
                message=map_error_to_message(output.stderr, {"id": snapshot_id}),
            )

        # Add to store on success
        snap_info = SnapshotInfo(
            snapshot=snapshot_id,
            message=message[:MSG_TRUNCATE_LEN] if message else "",
            metadata=metadata or {},
            created_at=datetime.now(timezone.utc).isoformat(),
        )
        self._store.add(snap_info)

        return CheckpointResult(
            success=True,
            message=f"Checkpoint created: {snapshot_id}",
            snapshot=snapshot_id,
        )

    def list_checkpoints(self) -> List[SnapshotInfo]:
        """List all snapshots for the workspace.

        Equivalent to: ws-ckpt list --workspace <ws> --format json

        Parses JSON output and updates the SnapshotStore.
        """
        if not self.is_cli_available():
            return self._store.get_all()

        output = self._run([
            "list",
            "--workspace", self._config.workspace_path,
            "--format", "json",
        ])

        if output.exit_code != 0:
            print(
                f"[ws-ckpt] list failed: {map_error_to_message(output.stderr)}",
                flush=True,
            )
            return self._store.get_all()

        # Parse JSON output from CLI
        try:
            data = json.loads(output.stdout)
            snapshots: List[SnapshotInfo] = []

            items = data if isinstance(data, list) else data.get("snapshots", [])
            for item in items:
                snapshots.append(SnapshotInfo(
                    snapshot=item.get("id", item.get("snapshot", "")),
                    message=item.get("message", ""),
                    metadata=item.get("metadata", {}),
                    created_at=item.get("created_at", item.get("createdAt", "")),
                ))

            self._store.set_all(snapshots)
        except (json.JSONDecodeError, KeyError, TypeError) as e:
            print(f"[ws-ckpt] Failed to parse list output: {e}", flush=True)

        return self._store.get_all()

    def get_status(self) -> CommandOutput:
        """Query daemon and/or workspace status.

        Equivalent to: ws-ckpt status [--workspace <ws>]
        """
        args = ["status"]
        if self._config.workspace_path:
            args.extend(["--workspace", self._config.workspace_path])
        return self._run(args)
